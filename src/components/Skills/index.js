            ...
            >
              ...
              <div className="flex flex-wrap gap-2 sm:gap-3 justify-center">
                {skill.skills.map((item, skillIndex) => (
                  <motion.div
                    key={skillIndex}
                    className="flex items-center gap-2 px-3 sm:px-4 py-2 bg-dark-700 rounded-full border border-dark-600 hover:border-primary-500/50 transition-all duration-300 group"
                    variants={itemVariants}
                    whileHover={{ 
                      scale: 1.1, 
          ...
          >
            ...
            <div className="flex flex-wrap justify-center gap-2 sm:gap-3">
              {['React', 'Node.js', 'TypeScript', 'AWS', 'Docker', 'Kubernetes', 'GraphQL', 'Next.js'].map((tech, index) => (
                <motion.span
                  key={tech}
                  className="px-3 sm:px-4 py-2 bg-gradient-to-r from-primary-500/20 to-accent-500/20 text-primary-400 rounded-full text-xs sm:text-sm font-medium border border-primary-500/30"
                  initial={{ opacity: 0, scale: 0 }}
                  animate={inView ? { opacity: 1, scale: 1 } : { opacity: 0, scale: 0 }}
